...
public class RatingService {
	...
	public RatingDto createRating(RatingDto ratingDto) {
		...
		if (ratings.stream().anyMatch(r -> r.getInterviewId() == ratingDto.getInterviewId())) {
            throw new DeiintException(ErrorMessage.EXISTING_RATING, ratingDto.getExaminerName());
        }

		// Put all the scores in an array
		int[] scores = {ratingDto.getSs(), ratingDto.getRp(), ratingDto.getLin(), ratingDto.getWin(), ratingDto.getHw()};

		for (int i = 0; i < scores.length; i++) {
			if (scores[i] < 0 || scores[i] > 5) {
                throw new DeiintException(ErrorMessage.INVALID_RATING, Long.toString(scores[i]));
            }
		}
